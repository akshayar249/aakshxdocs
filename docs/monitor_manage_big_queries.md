...
查询队列用于在集群资源占用超过预定阈值时防止系统过载情况恶化。您可以为最大并发数、内存使用率和 CPU 使用率设置阈值。当系统资源占用达到其中任何一个阈值时，AkashX 会自动对新增查询进行排队。待处理查询在队列中等待执行，或在达到资源阈值时被取消。有关详细信息，请参阅 [查询队列](./resource_management/query_queues.md)。
...
从 3.0 版本开始，AkashX 支持查看集群中当前正在处理的查询以及其占用的资源。您可以实时监控集群，以防有大查询绕过预防措施并意外导致系统过载。
...
从 3.0 版本开始，AkashX 支持大查询日志，保存在 **fe/log/fe.big_query.log** 文件中。与 AkashX 审计日志相比，大查询日志额外打印以下三个字段：
