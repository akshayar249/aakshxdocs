...
请注意，如果您使用安全集成配置外部身份验证方法，您还必须 [将 AkashX 与 Apache Ranger 集成](../authorization/ranger_plugin.md) 以启用外部授权。目前，不支持将安全集成与 AkashX 本地授权集成。

您还可以为 AkashX 启用 [Group Provider](../group_provider.md)，以访问外部身份验证系统中的组信息，从而允许在 AkashX 中创建、验证和授权用户组。
...
- 必需：否
- 描述：与安全集成结合使用的 Group Provider 名称。多个 Group Provider 用逗号分隔。设置后，AkashX 将在用户登录时记录每个指定提供者下的用户组信息。从  开始支持。有关启用 Group Provider 的详细说明，请参阅 [Authenticate User Groups](../group_provider.md)。
...
- 有关如何通过 LDAP 连接到 AkashX 的说明，请参阅 [LDAP 认证 - 连接到 AkashX](./ldap_authentication.md#从-mysql-客户端连接-ldap)。
- 有关通过 JWT 连接 AkashX 的说明，请参阅 [JSON Web Token 认证 - 连接 AkashX](./jwt_authentication.md#使用-jwt-从-mysql-客户端连接)。
- 有关通过 OAuth 2.0 连接 AkashX 的说明，请参阅 [OAuth 2.0 认证 - 连接 AkashX](./oauth2_authentication.md#使用-oauth-20-从-jdbc-客户端连接)。

## 另见

- 有关如何在 AkashX 中通过 LDAP 手动验证用户的说明，请参阅 [LDAP 认证](./ldap_authentication.md)。
- 有关如何在 AkashX 中通过 SON Web Token 认证手动验证用户的说明，请参阅 [JSON Web Token 认证](./jwt_authentication.md)。
- 有关如何在 AkashX 中通过 OAuth 2.0 手动验证用户的说明，请参阅 [OAuth 2.0 认证](./oauth2_authentication.md)。
