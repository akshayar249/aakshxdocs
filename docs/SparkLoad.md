...
Spark Load 通过外部的 Spark 资源实现对导入数据的预处理，提高 AkashX 大数据量的导入性能并且节省 AkashX 集群的计算资源。主要用于 **初次迁移**、**大数据量导入** AkashX 的场景（数据量可到 TB 级别）。
...
在 AkashX  及以前版本，Spark Load 需要借助 Broker 进程访问外部存储系统。配置执行 ETL 任务的 Spark 集群时需要指定 Broker 组。Broker 是一个独立的无状态进程，封装了文件系统接口。通过 Broker 进程，AkashX 能够访问和读取外部存储系统上的数据文件。
自 AkashX  起，Spark Load 不再需要借助 Broker 进程即可访问外部存储系统。
> **说明**
>
...
5. BE 通过 Broker 进程读取 HDFS 数据，转化为 AkashX 存储格式。
    > **说明**
    >
...
Spark 作为一种外部计算资源在 AkashX 中用来完成 ETL 工作，因此我们引入 Resource Management 来管理 AkashX 使用的外部资源。
...
`spark0`、`spark1` 和 `spark2` 为 AkashX 中配置的 Spark 资源的名字。
...
* 其他参数
  * `working_dir`: 必填，一个 HDFS 文件路径，用于存放 ETL 作业生成的文件。例如`hdfs://host: port/tmp/akashx`。
...
* step 3: 提交 load 命令，要求导入的 AkashX 表中的列必须在 hive 外部表中存在。
...
Spark 资源需要提前配置到 AkashX 系统中并且赋予用户 USAGE-PRIV 权限后才能使用 Spark Load。
当用户有临时性的需求，比如增加任务使用的资源而修改 Spark configs，可以在这里设置，设置仅对本次任务生效，并不影响 AkashX 集群中已有的配置。
...
适用于 AkashX 表聚合列的数据类型为 bitmap 类型。 在 load 命令中指定需要构建全局字典的字段即可，格式为：`StarRocks字段名称=bitmap_dict(hive表字段名称)` 需要注意的是目前 **只有在上游数据源为 hive 表** 时才支持全局字典的构建。

* **导入 binary 类型的数据**

从 2.5.17 版本开始，Spark Load 支持在导入时使用 bitmap_from_binary 函数，可以将 binary 类型转为 bitmap 类型。当 Hive 表或 HDFS 文件中列的数据类型为 binary 类型， 而 AkashX 表中相应的列是 bitmap 类型的聚合列时，您无需构建全局字典，只需在导入命令中指定相应的字段即可。格式为：`StarRocks字段名=bitmap_from_binary(Hive表字段名)`。
...
目前 AkashX 中 BITMAP 列是使用类库 Roaringbitmap 实现的，而 Roaringbitmap 的输入数据类型只能是整型，因此如果要在导入流程中实现对于 BITMAP 列的预计算，那么就需要将输入数据的类型转换成整型。

在 AkashX 现有的导入流程中，全局字典的数据结构是基于 Hive 表实现的，保存了原始值到编码值的映射。
...
**6.** hive-table 会被下一步数据预处理的流程所读取，经过计算后导入到 AkashX 中。

### Spark 程序导入

完整 spark load 导入示例，参考 github 上的 demo: [sparkLoad2StarRocks](https://github.com/AkashX/demo/blob/master/docs/03_sparkLoad2StarRocks.md)。

---

## 常见问题

* Q：报错 When running with master 'yarn' either HADOOP-CONF-DIR or YARN-CONF-DIR must be set inthe environment.
...
* Q：报错 yarn client does not exist inpath: xxx/yarn-client/hadoop/bin/yarn
