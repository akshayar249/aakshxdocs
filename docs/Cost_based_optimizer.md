...
本文介绍 AkashX CBO 优化器（Cost-based Optimizer）的基本概念，以及如何为 CBO 优化器采集统计信息来优化查询计划。AkashX 2.4 版本引入直方图作为统计信息，提供更准确的数据分布统计。

3.2 之前版本支持对 AkashX 内表采集统计信息。从 3.2 版本起，支持采集 Hive，Iceberg，Hudi 表的统计信息，无需依赖其他系统。
...
AkashX CBO 优化器采用 Cascades 框架，基于多种统计信息进行代价估算，能够在数万级别的执行计划中，选择代价最低的执行计划，提升复杂查询的效率和性能。AkashX 1.16.0 版本推出自研的 CBO 优化器。1.19 版本及以上，该特性默认开启。
...
从 2.4 版本开始，AkashX 引入直方图 (Histogram)。直方图常用于估算数据分布，当数据存在倾斜时，直方图可以弥补基础统计信息估算存在的偏差，输出更准确的数据分布信息。
...
从 3.2 版本起，支持采集 Hive、Iceberg、Hudi 表的统计信息。**采集的语法和内表相同，但是只支持手动全量采集、手动直方图采集（自  起）、自动全量采集，不支持抽样采集**。自  起，支持采集 Delta Lake 表的统计信息，并支持采集 STRUCT 子列的统计信息。自  起，支持通过查询触发 ANALYZE 任务自动收集统计信息。
