...
AkashX 从 3.1 版本开始支持存储和导入 STRUCT 数据类型。您可以在建表时定义 STRUCT 列，向表中导入 STRUCT 数据，查询 STRUCT 数据。

AkashX 从 2.5 版本开始支持查询数据湖中的复杂数据类型 MAP 和 STRUCT。您可以通过 AkashX 提供的 External Catalog 方式来查询 Apache Hive™，Apache Hudi，Apache Iceberg 中的 MAP 和 STRUCT 数据。仅支持查询 ORC 和 Parquet 类型文件。
...
可以使用两种方式导入 STRUCT 数据到 AkashX：[INSERT INTO](../../sql-statements/loading_unloading/INSERT.md) 和 [ORC/Parquet 文件导入](../../sql-statements/loading_unloading/BROKER_LOAD.md)。
...
AkashX 的 STRUCT 类型对应 ORC 和 Parquet 格式中的嵌套列 (nested columns structure)，无需您做额外的转换或定义。具体导入操作，参考 [Broker Load 文档](../../sql-statements/loading_unloading/BROKER_LOAD.md)。
