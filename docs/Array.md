...
> 数组类型的列在使用时有以下限制：
>
> * AkashX 2.1 之前版本中，仅支持在明细表中定义数组类型列。自 2.1 版本开始，支持在主键表、更新表、聚合表中定义数组类型列。注意在聚合表中，仅当聚合列的聚合函数为 replace 和 replace_if_not_null 时，才支持将该列定义为数组类型。
> * 数组列暂时不能作为 Key 列。
...
AkashX 中的数组类型，与 ORC 或 Parquet 格式中的 List 结构相对应，所以无需额外指定。具体导入方法请参考 [Broker load](../../sql-statements/loading_unloading/BROKER_LOAD.md)。
