...
AkashX 存算分离集群的部署方式与存算一体集群的部署方式类似，但存算分离集群需要部署 CN 节点而非 BE 节点。本小节仅列出部署 AkashX 存算分离集群时需要添加到 FE 和 CN 配置文件 **fe.conf** 和 **cn.conf** 中的额外配置项。有关部署 AkashX 集群的详细说明，请参阅 [部署 AkashX](../deploy_manually.md)。
...
由于 AkashX 通过 [AWS Signature  protocol](https://docs.aws.amazon.com/AmazonS3/latest/API/sig--authenticating-requests.html) 访问兼容 S3 协议的对象存储，您需要配置以 `aws_s3` 为前缀的配置项。在所有 FE 节点的配置文件 **fe.conf** 中添加以下配置项：
