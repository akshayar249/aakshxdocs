...
从  开始，[表达式分区](./expression_partitioning.md)方式进一步优化，以统一所有分区策略，并支持更复杂的解决方案。在大多数情况下，建议您使用表达式分区。表达式分区将在未来版本中逐渐取代 List 分区策略。
...
- AkashX 存算分离模式 从 3.1.1 版本开始支持该功能。
- 使用 `ALTER TABLE <table_name> DROP PARTITION <partition_name>;` 分区直接被删除并且不能被恢复。
- 自 、、 以及  起，AkashX 支持[备份与恢复](../../administration/management/Backup_and_restore.md) List 分区表。
