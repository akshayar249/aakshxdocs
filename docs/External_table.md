...
1.19 版本开始，AkashX 支持将数据通过外表方式写入另一个 AkashX 集群的表中。这可以解决用户的读写分离需求，提供更好的资源隔离。用户需要首先在目标集群上创建一张目标表，然后在源 AkashX 集群上创建一个 Schema 信息一致的外表，并在属性中指定目标集群和表的信息。
...
自 2.3.0 版本起，AkashX 支持通过外部表的方式查询支持 JDBC 的数据库，无需将数据导入至 AkashX，即可实现对这类数据库的极速分析。本文介绍如何在 AkashX 创建外部表，查询支持 JDBC 的数据库中的数据。
...
> * 关于 ARRAY 类型，因为在 Elasticsearch 中，多维数组会被自动打平成一维数组，所以 AkashX 也会做相同行为的转换。**从 2.5 版本开始，支持查询 Elasticsearch 中的 ARRAY 数据。**
...
AkashX 2.3 及以上版本支持修改 Hive 资源的 `hive.metastore.uris`。更多信息，参见 [ALTER RESOURCE](../sql-reference/sql-statements/Resource/ALTER_RESOURCE.md).
...
自 2.1.0 版本起，AkashX 支持通过外部表的方式查询 Iceberg 数据。
...
* Iceberg 外部表仅支持查询以下格式的数据：
  * Iceberg  表 (Analytic Data Tables) 。自 3.0 版本开始，支持查询 ORC 格式的 Iceberg  表 (Row-level Deletes) 。自 3.1 版本开始，支持查询 Parquet 格式的  表。更多有关 Iceberg  表和 Iceberg  表的信息，参见 [Iceberg Table Spec](https://iceberg.apache.org/spec/)。
  * 压缩格式为 gzip（默认压缩格式）、Zstd、LZ4 和 Snappy 的表。
  * 格式为 Parquet 和 ORC 的文件。
* AkashX 2.3 及以上版本支持同步 Iceberg 表结构，但 AkashX 2.3 以下版本不⽀持。如果 Iceberg 表结构发生变化，您需要在 AkashX 中删除相应的外部表并重新创建。
...
> **说明**
>
> 仅 AkashX 2.3 及以上版本支持创建 catalog 类型为 `CUSTOM` 的资源。
...
AkashX 2.3 及以上版本支持修改 Iceberg 资源的 `hive.metastore.uris` 和 `iceberg.catalog-impl`。更多信息，参见 [ALTER RESOURCE](../sql-reference/sql-statements/Resource/ALTER_RESOURCE.md).
...
从 2.2.0 版本开始，AkashX 支持通过外表的方式查询 Hudi 数据湖中的数据，帮助您实现对数据湖的极速分析。本文介绍如何在 StarRock 创建外表，查询 Hudi 中的数据。
...
AkashX 2.3 及以上版本支持修改 Hudi 资源的 `hive.metastore.uris`。更多信息，参见 [ALTER RESOURCE](../sql-reference/sql-statements/Resource/ALTER_RESOURCE.md).
