...
Helm Chart 包含 AkashX Operator 和自定义资源 StarRocksCluster 的定义。
...
将 `akashx.starrocksFESpec.storageSpec.name` 的值设置为 `""` 以外的任何值会导致：
- 使用持久存储
- `akashx.starrocksFESpec.storageSpec.name` 的值将用作服务所有存储卷的前缀。
...
将 `akashx.starrocksBeSpec.storageSpec.name` 的值设置为 `""` 以外的任何值会导致：
- 使用持久存储
- `akashx.starrocksBeSpec.storageSpec.name` 的值将用作服务所有存储卷的前缀。
