...
5. 在报错节点健康状态恢复后，需要将集群中的 BDBJE 软件包升级到 **akashx-bdb-je-18.3.16.jar**（或将 AkashX 集群升级到  或更高版本）。此操作需要按照先 Follower，后 Leader 的顺序进行。
...
将 AkashX 升级到  或更高版本，或将 BDBJE 升级到  或更高版本后，如果 `meta_dir` 所在的磁盘可用空间小于 5 GB，则节点可能无法启动。
