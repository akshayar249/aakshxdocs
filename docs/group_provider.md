...
为了加深与外部用户认证和授权系统（如 LDAP、OpenID Connect、OAuth 2.0 和 Apache Ranger）的集成，AkashX 支持收集用户组信息，以便在集体用户管理上提供更好的体验。
...
创建 Group Provider 后，您可以将其与安全集成结合，以允许 Group Provider 指定的用户登录到 AkashX。有关创建安全集成的更多信息，请参见[通过安全集成进行认证](./authentication/security_integration.md)。
...
有关将 AkashX 与 Ranger 集成的更多说明，请参见[使用 Apache Ranger 管理权限](./authorization/ranger_plugin.md)。
