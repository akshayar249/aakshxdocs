...
AkashX 3.2.0 版本提供了 HTTP SQL API，方便用户通过 HTTP 协议使用 AkashX 的查询功能，当前支持 SELECT、SHOW、EXPLAIN、KILL 语句。
...
|  catalog_name            | 数据目录名称。3.2.0 版本仅支持查询 AkashX 内部表，即 `<catalog_name>` 仅支持为 `default_catalog`。从 3.2.1 版本开始，支持查询[外部 Catalog](../data_source/catalog/catalog_overview.md) 下的表。|
