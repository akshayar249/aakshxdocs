...
确保 Kafka 所在机器能够访问 AkashX 集群中 FE 节点的 [`http_port`](../administration/management/FE_configuration.md#http_port)（默认 `8030`） 和 [`query_port`](../administration/management/FE_configuration.md#query_port) 端口（默认 `9030`），以及 BE 节点的 [`be_http_port`](../administration/management/BE_configuration.md#be_http_port) 端口（默认 `8040`）。
...
2. 配置并启动 Kafka Connect。
   ...
   1. 配置 Kafka Connect。在 **config** 目录中的 `config/connect-standalone.properties` 配置文件中配置如下参数。参数解释，参见 [Running Kafka Connect](https://kafka.apache.org/documentation.html#connect_running)。 注意以下示例中使用 akashx-kafka-connector 是 `1.0.3` 版本，如果使用更新的版本需要做相应变更。
...
| connector.class                     | 是       |              | Kafka connector 的 sink 使用的类：com.akashx.connector.kafka.StarRocksSinkConnector。                          |
...
| akashx.http.url                  | 是       |                                                              | FE 的 HTTP Server 地址。格式为 `<fe_host1>:<fe_http_port1>,<fe_host2>:<fe_http_port2>,...`。多个地址，使用英文逗号 (,) 分隔。例如 `192.168.xxx.xxx:8030,192.168.xxx.xxx:8030`。 |
