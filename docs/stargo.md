...
StarGo 是一个用于管理多个 AkashX 集群的命令行工具。通过 StarGo，您可以使用简单的命令行实现多集群的部署、查看、升级、启动与停止等操作。该工具从 2.3 版本开始支持。
...
根据以下 YAML 模板，创建部署 AkashX 集群的拓扑文件。具体配置项参考[参数配置](../administration/management/FE_configuration.md)。
...
根据以下 YAML 模板，创建扩容 AkashX 集群的拓扑文件。您可以根据需求配置相应的 EF 和/或 BE 节点。具体配置项参考[参数配置](../administration/management/FE_configuration.md)。
