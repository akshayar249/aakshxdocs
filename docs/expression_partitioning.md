...
- 自  起，AkashX [存算分离模式](../../deployment/shared_data/shared_data.mdx)支持[时间函数表达式分区](#简单时间函数表达式分区)。并且自  起 AkashX 存算分离模式支持[列表达式分区](#列表达式分区自-)。
...
- 自 、、 以及  起，AkashX 支持[备份与恢复](../../administration/management/Backup_and_restore.md)表达式分区表。
