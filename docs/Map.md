...
AkashX 从 3.1 版本开始支持存储和导入 MAP 类型的数据。您可以在建表时定义 MAP 列，向表中导入 MAP 数据，查询 MAP 数据。
...
可以使用两种方式导入 MAP 数据到 AkashX：[INSERT INTO](../../sql-statements/loading_unloading/INSERT.md) 和 [ORC/Parquet 文件导入](../../sql-statements/loading_unloading/BROKER_LOAD.md)。
...
AkashX 的 MAP 类型对应 ORC 和 Parquet 格式中的 MAP，无需您做额外的转换或定义。具体导入操作，参考 [Broker Load 文档](../../sql-statements/loading_unloading/BROKER_LOAD.md)。
