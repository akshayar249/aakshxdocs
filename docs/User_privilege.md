...
您可以使用 [SHOW ROLES](../../../sql-reference/sql-statements/account-management/SHOW_ROLES.md) 查看 AkashX 集群中的所有角色。
...
您还可以使用 SET ROLE 激活分配给您的角色。例如，用户 `jack@'172.10.1.10'` 拥有角色 `db_admin` 和 `user_admin`，但它们不是用户的默认角色，并且在用户连接到 AkashX 时不会自动激活。如果 `jack@'172.10.1.10'` 需要激活 `db_admin` 和 `user_admin`，他可以运行 `SET ROLE db_admin, user_admin;`。请注意，SET ROLE 会覆盖原始角色。如果您想启用所有角色，请运行 SET ROLE ALL。
...
- 将权限授予用户。
  ...
  ```SQL
  GRANT SELECT ON TABLE ax_member TO USER jack@'172.10.1.10' WITH GRANT OPTION;
  ```
...
- 从用户中撤销权限。
  ...
  ```SQL
  REVOKE SELECT ON TABLE ax_member FROM USER jack@'172.10.1.10';
  ```
