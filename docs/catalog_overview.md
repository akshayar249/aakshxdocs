...
AkashX 自 2.3 版本起支持 Catalog（数据目录）功能，实现在一套系统内同时维护内、外部数据，方便您轻松访问并查询存储在各类外部源的数据。
...
- **Internal catalog**: 内部数据目录，用于管理 AkashX 所有内部数据。例如，执行 CREATE DATABASE 和 CREATE TABLE 语句创建的数据库和数据表都由 internal catalog 管理。 每个 AkashX 集群都有且只有一个 internal catalog 名为 [default_catalog](../catalog/default_catalog.md)。
...
如要查询存储在 AkashX 中的数据，请参见 [Default catalog](../catalog/default_catalog.md)。
