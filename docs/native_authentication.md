...
AkashX 本地身份验证是一种基于密码的身份验证方法。此外，AkashX 还支持与外部身份验证系统集成，如 LDAP、OpenID Connect 和 OAuth 2.0。更多说明，请参见[Authenticate with Security Integration](./security_integration.md)。
...
用户的默认角色在连接到 AkashX 时会自动激活。有关如何在连接后为用户启用所有（默认和授予的）角色的说明，请参见[启用所有角色](../authorization/User_privilege.md#启用所有角色)。
