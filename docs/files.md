...
控制系统导出数据的 Parquet 版本。从  开始支持。有效值：`1.0`、`2.4` 和 `2.6`（默认）。此属性仅支持数据导出。
...
- 要选择服务主体身份验证方法，请按如下配置 `StorageCredentialParams`：
  ...
  | `azure.adls2.oauth2_client_endpoint` | 是          | 服务主体或应用程序的 OAuth 2.0 令牌终端（）。 |
...
- 要选择服务主体身份验证方法，请按如下配置 `StorageCredentialParams`：
  ...
  | `azure.adls1.oauth2_endpoint`   | 是          | 服务主体或应用程序的 OAuth 2.0 令牌终端（）。 |
