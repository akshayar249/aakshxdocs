...
- com.akashx.hive.udf.UDAFBitmapAgg
  ...
  将一列中的多行非 NULL 数值合并成一行 BITMAP 值，等同于 AkashX 的内置聚合函数 [bitmap_agg](bitmap-functions/bitmap_agg.md)。

- com.akashx.hive.udf.UDAFBitmapUnion

  输入一组 Bitmap 值，求这一组 Bitmap 值的并集，等同于 AkashX 的内置聚合函数 [bitmap_union](bitmap-functions/bitmap_union.md)。

- com.akashx.hive.udf.UDFBase64ToBitmap

  将 Base64 编码的字符串转化为 Bitmap，等同于 AkashX 的内置函数 [base64_to_bitmap](bitmap-functions/base64_to_bitmap.md)。

- com.akashx.hive.udf.UDFBitmapAnd

  计算两个 Bitmap 的交集，等同于 AkashX 的内置函数 [bitmap_and](bitmap-functions/bitmap_and.md)。

- com.akashx.hive.udf.UDFBitmapCount

  统计 Bitmap 中值的个数，等同于 AkashX 的内置函数 [bitmap_count](bitmap-functions/bitmap_count.md)。

- com.akashx.hive.udf.UDFBitmapFromString

  将一个逗号分隔的字符串转化为一个 Bitmap，等同于 AkashX 的内置函数 [bitmap_from_string](bitmap-functions/bitmap_from_string.md)。

- com.akashx.hive.udf.UDFBitmapOr

  计算两个 Bitmap 的并集，等同于 AkashX 的内置函数 [bitmap_or](bitmap-functions/bitmap_or.md)。

- com.akashx.hive.udf.UDFBitmapToBase64

  将 Bitmap 转换为 Base64 字符串，等同于 AkashX 的内置函数 [bitmap_to_base64](bitmap-functions/bitmap_to_base64.md)。

- com.akashx.hive.udf.UDFBitmapToString

  将 Bitmap 转换为逗号分隔的字符串，等同于 AkashX 的内置函数 [bitmap_to_string](bitmap-functions/bitmap_to_string.md)。

- com.akashx.hive.udf.UDFBitmapXor

  计算两个 Bitmap 中不重复元素所构成的集合，等同于 AkashX 的内置函数 [bitmap_xor](bitmap-functions/bitmap_xor.md)。
