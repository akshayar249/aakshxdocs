...
- AkashX 采用 MySQL 协议进行通信，用户可通过 MySQL Client 或者 JDBC 连接到 AkashX 集群。选择 MySQL Client 版本时建议采用 5.1 及之后的版本，5.1 之前版本不支持长度超过 16 个字符的用户名。
...
- VARCHAR 最大长度：
  - 对于 AkashX 2.1 之前的版本，长度范围为 1~65533 字节。
  - 【公测中】自 AkashX 2.1 版本开始，长度范围为 1~1048576 字节。1048578（行最大值）- 2（长度标识位，记录实际数据长度）= 1048576。
