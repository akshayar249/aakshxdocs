...
从v3.0起，可以修改主键表的排序键。.3扩展了对明细表、聚合表和更新表的支持。
...
- 在嵌套在STRUCT列中的STRUCT字段中添加或删除子字段。
  ...
  假设有一个列`fx stuct<c1 int, c2 struct < int,  int>>`。在`c2`下添加``字段的语法是：

  ```SQL
  ALTER TABLE tbl MODIFY COLUMN fx ADD FIELD c2.v3 INT
  ```

  操作后，列变为`fx stuct<c1 int, c2 struct < int,  int,  int>>`。

- 在嵌套在ARRAY字段中的每个STRUCT字段中添加或删除子字段。

  假设有一个列`fx struct<c1 int, c2 array<struct < int,  int>>>`。字段`c2`是一个ARRAY类型，其中包含一个具有两个字段``和``的STRUCT。向嵌套的STRUCT中添加``字段的语法是：

  ```SQL
  ALTER TABLE tbl MODIFY COLUMN fx ADD FIELD c2.[*].v3 INT
  ```

  操作后，列变为`fx struct<c1 int, c2 array<struct < int,  int,  int>>>`。
...
从  起，StarRocks提供了一个SQL接口，用户可以通过运行SQL命令手动执行 Compaction 。他们可以选择特定的表或分区进行 Compaction。这提供了对 Compaction 过程的更多灵活性和控制。
...
1. 基于基础索引（k1,k2,k3,,）创建 Rollup `example_rollup_index`。使用列存储。
...
2. 基于 `example_rollup_index(k1,k3,,)` 创建 Rollup `example_rollup_index2`。
...
3. 基于基础 Rollup（k1, k2, k3, ）创建 Rollup `example_rollup_index3`。超时时间设置为一小时。
...
10. 重新排序`example_rollup_index`中的列。原始列顺序为k1, k2, k3, , 。
