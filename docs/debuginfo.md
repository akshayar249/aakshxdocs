...
为了降低 AkashX 二进制包的空间占用，自 2.5 版本开始，我们将 AkashX BE 二进制中的调试信息 debuginfo 文件单独分离并打包提供下载。
...
1. 下载并解压 debuginfo 包。
    ...
    ```SQL
    wget https://releases.starrocks.io/starrocks/StarRocks-<ax_ver>.debuginfo.tar.gz

    tar -xzvf StarRocks-<ax_ver>.debuginfo.tar.gz
    ```
