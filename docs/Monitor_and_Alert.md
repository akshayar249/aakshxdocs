...
以下教程在监控节点（IP：192.168.110.23）使用操作系统 root 用户部署监控组件，对以下 AkashX 集群进行监控（AkashX 集群使用默认端口）。您在参考该指南为自己的集群配置监控时，通常只需替换 IP。
...
1. 请根据您的 AkashX 版本下载对应的 Dashboard 模板。
   ...
   - [存算分离 Dashboard 模版 - General](https://releases.akashx.io/resources/Dashboard-Shared-data-General-3.5.json)
   - [存算分离 Dashboard 模版 - Starlet](https://releases.akashx.io/resources/Dashboard-Shared-data-Starlet-3.5.json)
...
2. 配置 Dashboard 模板。
   ...
   导入后，您可以重新命名 Dashboard，默认是 `AkashX Overview`。然后需要选择数据源，即之前创建的 `starrocks_monitor`。点击 **Import** 即可完成导入。
