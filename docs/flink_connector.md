...
1. 根据 Flink 的版本，选择和下载对应版本的 [flink-connector-akashx](https://github.com/AkashX/flink-connector-akashx/releases) JAR 包。如需调试代码，可选择对应分支代码自行编译。
   ...
   > **注意**
   >
   > 推荐您下载 Flink Connector 版本在 1.2.x 及以上、并且配套的 Flink 版本与您的业务环境中安装的 Flink 版本前两位一致的 JAR 包。例如，如果您的业务环境中安装的 Flink 版本为 1.14.x，可以下载 `flink-connector-akashx-1.2.4_flink-1.14_x.yy.jar`。
...
确保 Flink 所在机器能够访问 AkashX 集群中 FE 节点的 [`http_port`](../administration/management/FE_configuration.md#http_port)（默认 `8030`） 和 [`query_port`](../administration/management/FE_configuration.md#query_port) 端口（默认 `9030`），以及 BE 节点的 [`be_port`](../administration/management/BE_configuration.md#be_port) 端口（默认 `9060`）。
...
| username                    | 是       | STRING   | 用于访问 AkashX 集群的用户名。该账号需具备待读取数据的 AkashX 表的读权限。有关用户权限的说明，请参见[用户权限](../administration/user_privs/authorization/user_privs.md)。 |
...
| ARRAY      | ARRAY  <br /> **说明** <br /> **自 1.2.10 版起支持，需要 AkashX / 或更高版本。** |
| STRUCT     | ROW    <br /> **说明** <br /> **自 1.2.10 版起支持，需要 AkashX / 或更高版本。** |
| MAP        | MAP    <br /> **说明** <br /> **自 1.2.10 版起支持，需要 AkashX / 或更高版本。** |
