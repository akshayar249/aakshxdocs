...
- 此功能从3.3.0版本开始提供。在  之前默认禁用，需要手动启用。从  版本起，默认启用。
...
1. 修改 BE 配置：`enable_json_flat`，在  版本之前默认为 `false`。修改方法参考 [配置 BE 参数](../administration/management/BE_configuration.md#configure-be-parameters)。
2. 启用FE分区裁剪功能：
...
- `cbo_json_v2_rewrite`（默认：true）：启用 JSON  路径改写，将 `get_json_*` 等函数改写为直接访问 Flat JSON 子列，从而启用谓词下推和列裁剪。
