...
AkashX 中的同步物化视图仅能基于 [Default Catalog](../data_source/catalog/default_catalog.md) 中的单个基表创建，是一种特殊的查询加速索引。

自 2.4 版本起，AkashX 支持**异步物化视图**，可以基于多个基表创建，且支持更丰富的聚合函数。详细信息，请参阅 [异步物化视图](async_mv/Materialized_view.md)。
...
下表从支持的特性角度比较了 AkashX 2.5、2.4 中的异步物化视图以及同步物化视图（Rollup）：
...
- **基表（Base Table）**
  ...
  对于 AkashX 的同步物化视图，基表仅可以是 [Default catalog](../data_source/catalog/default_catalog.md) 中的单个内部表。AkashX 支持在明细表、聚合表上创建同步物化视图。
