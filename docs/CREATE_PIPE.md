...
AkashX 自 3.2.3 版本起支持 CREATE [OR REPLACE] PIPE。使用 CREATE [OR REPLACE] PIPE 时，如果指定的 `pipe_name` 与当前数据库里的某个已有的 Pipe 名称相同，则新建的 Pipe 会取代已有的 Pipe。
...
该示例以基于 IAM User 的认证鉴权方式为例，并假设 Parquet 源文件与 AkashX 目标表的结构相同。有关认证方式和语句详情，参见[配置 AWS 认证信息](../../../../integrations/authenticate_to_aws_resources.md) [FILES](../../../sql-functions/table-functions/files.md)。
