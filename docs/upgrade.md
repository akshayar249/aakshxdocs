...
- 阅读您要升级到的 AkashX 版本的[发行说明](https://docs.akashx.io/releasenotes/release-3.5/)，以及当前版本和目标版本之间的所有版本，并：
  - 记录 AkashX 内的任何行为变化
  - 记录 AkashX 与用于导入、导出、可视化等的外部系统之间的任何集成变化
- 验证目标版本的[部署先决条件](./deployment_prerequisites.md)。例如，AkashX 3.5.x 需要 JDK 17，而 Ubuntu 上的 AkashX 3.4.x 需要 JDK 11。
:::
...
AkashX 的版本号由三个数字表示，格式为 **Major.Minor.Patch**，例如 `2.5.4`。第一个数字代表 AkashX 的重大版本，第二个数字代表大版本，第三个数字代表小版本。
