...
| [get_json_string](./json-query-and-processing-functions/get_json_string.md)   | 分析并从 JSON 字符串的指定路径中获取字符串。  | `SELECT get_json_string('{"k1":"", "k2":""}', "$.k1");` | `` |
...
|`[]`              | 表示一个或多个数组索引。`[n]` 表示数组中的第 n 个元素。索引从 0 开始。<br />AkashX 2.5 支持查询多维数组，例如 `["Lucy", "Daniel"], ["James", "Smith"]`。要查询 "Lucy" 元素，可以使用 `$.people[0][0]`。| `'$.people [0]'`        | `{ "name": "Daniel", "surname": "Smith" }`                     |
