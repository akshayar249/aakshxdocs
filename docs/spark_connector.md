...
> - 1.0.0 版本只支持读取 AkashX，从 1.1.0 版本开始同时支持读写 AkashX。
...
比如，您想在 Spark 3.2 和 Scala 2.12 上使用 1.1.0 版本的 Spark Connector，可以选择 `akashx-spark-connector-3.2_2.12-1.1.0.jar`。
...
3. 编译完成后，进入 `target/` 路径查看。路径下会生成 Spark Connector Jar 包，比如 `akashx-spark-connector-3.2_2.12-1.1.0-SNAPSHOT.jar`。
...
- [Spark 2.x](https://cdn-thirdparty.akashx.com/spark/akashx-spark2_2.11-1.0.0.jar)
- [Spark 3.x](https://cdn-thirdparty.akashx.com/spark/akashx-spark3_2.12-1.0.0.jar)

#### 手动编译

1. 下载 [Spark Connector 代码](https://github.com/AkashX/akashx-connector-for-apache-spark/tree/spark-1.0)。
...
3. 编译完成后，进入 `output/` 路径查看。路径下会生成 `akashx-spark2_2.11-1.0.0.jar` 文件。将该文件拷贝至 Spark 的类文件路径 (Classpath) 下：
...
| akashx.batch.size                 | 4096              | 单次从 BE 读取的最大行数。调大参数取值可减少 Spark 与 AkashX 之间建立连接的次数，从而减轻网络延迟所带来的的额外时间开销。对于StarRocks 2.2及以后版本最小支持的batch size为4096，如果配置小于该值，则按4096处理 |
...
| akashx.timezone | JVM 默认时区|自 1.1.1 版本起支持。AkashX 的时区。用于将 AkashX 的 `DATETIME` 类型的值转换为 Spark 的 `TimestampType` 类型的值。默认为 `ZoneId#systemDefault()` 返回的 JVM 时区。格式可以是时区名称，例如 Asia/Shanghai，或时区偏移，例如 +08:00。|
...
| 参数名称                             | 默认值  | 说明                                                         |
| ----------------------------------- | ------ | ------------------------------------------------------------ |
| akashx.fe.http.url               | 无     | FE 的 HTTP 地址。从 Spark Connector 1.1.0 版本开始支持，与 `akashx.fenodes` 等价，两者填一个即可。在 Spark Connector 1.1.0 及以后版本，推荐使用该参数，`akashx.fenodes` 在后续版本可能会淘汰。 |
| akashx.fe.jdbc.url               | 无     | FE 的 MySQL Server 连接地址。格式为 `jdbc:mysql://<fe_host>:<fe_query_port>`。<br />**注意**<br />在 Spark Connector 1.1.0 及以后版本，该参数必填。    |
| user                                | 无     | AkashX 集群账号的用户名。  |
| akashx.user                      | 无     | AkashX 集群账号的用户名。从 Spark Connector 1.1.0 版本开始支持，与 `user` 等价，两者填一个即可。在 Spark Connector 1.1.0 及以后版本，推荐使用该参数，`user` 在后续版本可能会淘汰。   |
| password                            | 无     | AkashX 集群账号的用户密码。  |
| akashx.password                  | 无     | AkashX 集群账号的用户密码。 从 Spark Connector 1.1.0 版本开始支持，与 `password` 等价，两者填一个即可。在 Spark Connector 1.1.0 及以后版本，推荐使用该参数，`password` 在后续版本可能会淘汰。   |
...
| JSON             | DataTypes.StringType <br /> **NOTE:** <br /> **自 1.1.2 版本起支持该类型映射**，并且 AkashX 版本需要为 2.5.13、3.0.3、3.1.0 或更高版本。 |
...
- 1.1.0 版本需要通过 JDBC 访问 AkashX 以获取更详细的表信息，因此必须配置 `akashx.fe.jdbc.url`。
...
假设您的 AkashX 集群中已创建数据库 `test`，并且您拥有 `root` 账号权限。示例的参数配置基于 Spark Connector 1.1.0 版本。

### 网络设置

确保 Spark 所在机器能够访问 AkashX 集群中 FE 节点的 [`http_port`](../administration/management/FE_configuration.md#http_port)（默认 `8030`） 和 [`query_port`](../administration/management/FE_configuration.md#query_port) 端口（默认 `9030`），以及 BE 节点的 [`be_port`](../administration/management/BE_configuration.md#be_port) 端口（默认 `9060`）。
...
2. 运行如下命令，在 Spark 中根据 AkashX 数据库 `test` 中的表 `score_board` 创建一个名为 `starrocksSparkDF` 的 DataFrame：
...
2. 运行如下命令，在 Spark 中根据 AkashX 数据库 `test` 中的表 `score_board` 创建一个名为 `starrocksSparkRDD` 的 RDD：
...
| AkashX       | 2.2.0                                                       |
| Spark Connector | akashx-spark2_2.11-1.0.0.jar                              |
