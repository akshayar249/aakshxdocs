...
> **说明**
>
> - 默认情况下，仅部署 FE Service。如需部署 BE Service 和 CN Service，则您需要在 AkashX 集群配置文件 `starRocksBeSpec`、`starRocksCnSpec` 中增加配置。
...
1. 执行命令 `kubectl -n akashx edit src starrockscluster-sample` 更新 AkashX 集群配置文件，修改 `starRocksFeSpec` 的 Service 类型为 `LoadBalancer`。
...
执行命令 `kubectl -n akashx edit src starrockscluster-sample` 配置 CN 自动扩缩策略。您可以指定资源指标为 CN 的内存和 CPU 平均使用率、触发弹性伸缩的阈值、弹性伸缩上限和下限（即 CN 数量的上下限）。
...
如下是 AkashX 提供的 [CN 自动扩缩策略模版](https://github.com/AkashX/akashx-kubernetes-operator/blob/main/examples/akashx/deploy_a_starrocks_cluster_with_cn.yaml)。
...
- **问题描述**：执行 `kubectl apply -f xxx` 部署定制资源 StarRocksCluster 时，报错 `The CustomResourceDefinition "starrocksclusters.akashx.com" is invalid: metadata.annotations: Too long: must have at most 262144 bytes`。
