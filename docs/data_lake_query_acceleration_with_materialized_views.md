...
AkashX 提供了开箱即用的数据湖查询功能，非常适用于对湖中的数据进行探查式查询分析。在大多数情况下，[Data Cache](../../../data_source/data_cache.md) 可以提供 Block 级文件缓存，避免由远程存储抖动和大量I/O操作引起的性能下降。
...
对于 Iceberg Catalog, 从  版本开始，AkashX 支持检测分区级别的数据更改，当前只支持 Iceberg  表。
...
因此，您可以利用 AkashX 的 [View Delta Join 改写](query_rewrite_with_materialized_views.md#view-delta-join-改写) 能力来构建物化视图，对 `lineorder`、`dates`、`part` 和 `supplier` 进行 Join。
