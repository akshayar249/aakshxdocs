...
> 从  开始，AkashX 支持为使用 INSERT INTO FILES() 和 Broker Load 的数据导入提供 Query Profile。有关涉及的指标详情，请参见 [OlapTableSink Operator](./query_profile_operator_metrics.md#olaptablesink-operator)。
...
大多数用户可能会发现直接分析原始文本具有挑战性。AkashX 提供了一种 [基于文本的 Query Profile 可视化分析](./query_profile_text_based_analysis.md) 方法，以便更直观地理解。
